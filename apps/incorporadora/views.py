...
def unidade_import_empreendimento_csv(request, empreendimento_pk):
    ...
    if modo == 'substituir':
        from apps.propostas.models import UnidadeProposta
        unidade_ids = list(Unidade.objects.filter(bloco__empreendimento=empreendimento).values_list('id', flat=True))
        ItemTabelaVendas.objects.filter(unidade_id__in=unidade_ids).delete()
        UnidadeProposta.objects.filter(unidade_id__in=unidade_ids).delete()
